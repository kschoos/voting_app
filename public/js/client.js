...
function createNewPoll() {
  ...
  $(".content-header h4").text("'Creativity takes courage.' ― Henri Matisse").append("<br>").append("'That's why I like copy-pasting quotes.' - Myself");
  $(".content").text("");

  $("<form>").appendTo($(".content")).on("submit", sendPoll);

  $("<div>").appendTo($(".content form")).addClass("form-group").append("Title:");
  $("<input>").appendTo($(".content .form-group")).addClass("form-control form-control-lg").attr("type", "text").attr("data-toggle", "popover").attr("data-trigger", "focus").attr("data-placement", "left").attr("title", "Title").attr("name", "title").attr("data-content", "Tell me your name and I tell you who you are.").on("mouseover", function (e) {
    $(this).popover();
  });
  $(".content .form-group").append("Choices:");
  $("<textarea>").appendTo($(".content .form-group")).addClass("form-control").css("resize", "vertical").attr("data-toggle", "popover").attr("data-trigger", "focus").attr("data-placement", "left").attr("title", "Choices").attr("name", "choices").attr("data-content", "Add the choices for your poll and seperate them by new lines! Also take note of all the beautiful exclamation marks!").on("mouseover", function (e) {
    $(this).popover();
  });
  $("<input>").appendTo($(".content .form-group")).attr("type", "submit").addClass("btn btn-default btn-custom");
...
function showPoll(id) {
  ...
  $.ajax({
    ...
    success: function success(poll) {
      ...
      $(".content-header h1").text(poll.name.split("").map(function (letter, i) {
        return i === 0 ? letter.toUpperCase() : letter;
      }).join("")); // Set the header text
      $(".content-header h4").text("Polls are polls are polls are polls.");
      ...
      $("<form class='choice-form'>").appendTo($(".content")).on("submit", function (e) {
        ...
        $.ajax({
          ...
          type: "post",
          data: dataobj,
          success: function success(newPoll) {
